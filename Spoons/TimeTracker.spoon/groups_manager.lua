...
function M.deleteGroup(groupsPath, csvPath, groups, pathToDelete)
    ...
    for _, group in ipairs(groups) do
        -- Delete exact match or any subpaths
        if group ~= pathToDelete and not group:find("^" .. pathToDelete:gsub("%p", "%%%1") .. "::") then
...
function M.deleteTimeEntries(csvPath, pathToDelete)
    ...
    local firstLine = true
    local deletedCount = 0
    local escapePattern = pathToDelete:gsub("%p", "%%%1")
    ...
    for line in inputFile:lines() do
        if firstLine then
            ...
            -- Keep line if it doesn't match the path or any subpath
            if pathInLine and pathInLine ~= pathToDelete and not pathInLine:find("^" .. escapePattern .. "::") then
                outputFile:write(line .. "\n")
            else
                deletedCount = deletedCount + 1
            end
