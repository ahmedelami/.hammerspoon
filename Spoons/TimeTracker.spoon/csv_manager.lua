...
function M.initCSV(csvPath)
    ...
    if not file then
        ...
        if file then
            file:write("Start Time,End Time,Duration (minutes),Path,Description\n")
            file:close()
        end
    else
        file:close()
...
function M.logToCSV(csvPath, startTime, endTime, path, description)
